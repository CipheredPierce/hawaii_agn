...
def _qso_to_scalar_dict(qso, pad_columns: list[str] | None = None) -> dict:
    ...
    Host decomposition columns (SN_host, gal_par_N, qso_par_N, etc.) are
    only present in conti_result when decompose_host=True succeeded.
    pad_columns is the full list of expected host column names (built from
    npca_gal and npca_qso at start_run time) — any that are missing from
    this object's results are filled with NaN to keep the schema consistent.
    ...
    # Pad all expected host columns with NaN where absent.
    # pad_columns is pre-computed at start_run() from npca_gal / npca_qso
    # so it covers gal_par_N and qso_par_N even when this object was not
    # decomposed and host_result_name is empty.
    for name in (pad_columns or _HOST_RESULT_NAMES):
        if name not in result:
            result[name] = np.nan
...
class QSOCatalog:
    ...
    def append(
        ...
        # Resolve objid
        if objid is None:
            objid = getattr(qso, 'sdss_name', None) or None
        ...
        if store_spectra:
            if objid is None:
                warnings.warn(
                    "store_spectra=True but no objid could be determined — "
                    "skipping spectrum storage.",
                    stacklevel=2,
                )
            else:
                self._store_spectrum(qso, objid)
...
-----
    ...
    # Call once before the fitting loop
    cat.start_run(label="night1")
    cat_full.start_run(label="night1")
...
class QSOCatalog:
    ...
    def start_run(self, label: str | None = None) -> str:
        """
        Initialise a new bulk-fit run. Call once before the fitting loop.
...
pyqsofit_catalog.py
-------------------
A wrapper around pandas HDFStore / h5py for accumulating PyQSOFit results
into a hierarchical HDF5 catalog.
...
---------
    /YYYYMMDD/                          ← date group  (fit run date)
        YYYYMMDD_HHMMSS[_label]/        ← bulk_fit group (one per run)
            ...
            spectra/                    ← (full catalog only)
                <objid>/
                    wave, flux, err, conti, line, [extras]
                    attrs: ra, dec, redshift
...
-----
    ...
    # Start a run (call once before the fitting loop)
    cat.start_run(label="my_run")          # optional label
    cat_full.start_run(label="my_run")

    # After each fit:
    cat.append(qso.result, objid="J1234+5678")

    cat_full.append(
        qso.result, objid="J1234+5678",
        wave=qso.wave, flux=qso.flux, err=qso.err,
        conti=qso.conti, line=qso.line,
    )

    # Query / load (scoped to current run):
    df = cat.load()
    df_highz = cat.query("redshift > 2.0")

    # Load from a specific past run:
    df = cat.load(run_key="/20250601/20250601_143000_my_run")

    # Export the current run to FITS:
    cat.to_fits("run_results.fits")
...
def _sanitize_result(result: dict) -> dict:
    """
    Convert numpy scalars / 0-d arrays to plain Python types and replace
    None values with NaN so pandas doesn't complain.
    """
    clean = {}
    for k, v in result.items():
        if v is None:
...
def _make_run_name(label: str | None = None) -> tuple[str, str]:
    """
    Build (date_key, run_key) from the current system time.
    ...
    date_key : str
        e.g. ``"20250601"``
    run_name : str
        e.g. ``"20250601_143000"`` or ``"20250601_143000_my_run"``
    """
    now = datetime.datetime.now()
    date_key = now.strftime("%Y%m%d")
    run_name = now.strftime("%Y%m%d_%H%M%S")
...
class QSOCatalog:
    """
    Append-friendly HDF5 catalog for PyQSOFit results.
    ...
    The internal hierarchy is::
        ...
        /YYYYMMDD/
            YYYYMMDD_HHMMSS[_label]/
                params          ← pandas scalar table (one row per object)
                spectra/        ← h5py arrays per object (full catalog only)
                    <objid>/
                        wave, flux, err, conti, line, [extras]
    ...
    def __init__(self, path: str):
        ...
        self._date_key: str | None = None
        self._run_name: str | None = None
        self._param_key: str | None = None   # full HDF5 key for scalar table
        self._spectra_prefix: str | None = None  # h5py group prefix for spectra
    ...
    def start_run(self, label: str | None = None) -> str:
        """
        Initialise a new bulk-fit run.  Must be called once before the
        fitting loop begins.

        Parameters
        ----------
        label : str, optional
            A short human-readable label appended to the run name,
            e.g. ``"night1"`` → ``"20250601_143000_night1"``.

        Returns
        -------
        str
            The full run key, e.g. ``"/20250601/20250601_143000_night1"``.
        """
        self._date_key, self._run_name = _make_run_name(label)
        self._param_key   = f"{self._date_key}/{self._run_name}/params"
        self._spectra_prefix = f"{self._date_key}/{self._run_name}/spectra"
        self._min_itemsize = {}
        print(f"Run started → /{self._date_key}/{self._run_name}")
        return f"/{self._date_key}/{self._run_name}"

    def _require_run(self) -> None:
        """Raise if start_run() hasn't been called yet."""
        if self._param_key is None:
            raise RuntimeError(
                "No active run.  Call start_run() before append()."
            )
    ...
    def append(
        ...
        ----------
        result : dict
            Scalar output dict from PyQSOFit (ra, dec, redshift, …).
        objid : str, optional
            Unique identifier for this object (e.g. ``"J1234+5678"``).
            Required if you want to store spectral arrays.
        wave, flux, err : np.ndarray, optional
            Observed wavelength / flux / uncertainty arrays.
        conti : np.ndarray, optional
            Best-fit continuum model (same length as wave).
        line : np.ndarray, optional
            Best-fit emission-line model (same length as wave).
        extra_arrays : dict of {str: np.ndarray}, optional
            Any additional 1-D arrays to store alongside the spectrum.
        """
        self._require_run()
        ...
        # ---- spectra / arrays ----------------------------------------
        has_arrays = any(
            a is not None for a in [wave, flux, err, conti, line, extra_arrays]
        )
        if objid is not None and has_arrays:
            self._store_spectrum(
                objid,
                wave=wave, flux=flux, err=err,
                conti=conti, line=line,
    ...
    def _store_spectrum(
        self, objid: str, *,
        wave, flux, err, conti, line,
        extra_arrays: dict,
        attrs: dict,
    ) -> None:
        """Write per-object arrays into the spectra sub-group of the active run."""
        self._require_run()
        grp_path = f"{self._spectra_prefix}/{objid}"
        ...
        with h5py.File(self.path, "a") as f:
            ...
            for name, arr in arrays.items():
                if arr is not None:
                    grp.create_dataset(
                        name, data=np.asarray(arr, dtype="f8"),
    ...
    def _rebuild_with_new_row(self, df_new: pd.DataFrame) -> None:
        """
        Fallback for when a string column needs more space than originally
        allocated: load existing table, concatenate, and overwrite.
        """
        warnings.warn(
            "String column width increased — rebuilding param table. "
    ...
    # ------------------------------------------------------------------
    # Reading  (all methods accept an optional run_key override)
    # ------------------------------------------------------------------

    def _resolve_param_key(self, run_key: str | None) -> str:
        """
        Return the HDF5 key for the scalar table.

        If ``run_key`` is provided it overrides the active run, allowing
        reads from any historical run in the file.  ``run_key`` should be
        the run path returned by ``start_run()``, e.g.
        ``"/20250601/20250601_143000_my_run"``.
        """
        if run_key is not None:
            return run_key.lstrip("/") + "/params"
        self._require_run()
    ...
    def load(
        ...
        columns : list of str, optional
            Subset of columns to load.  Loads all if omitted.
        run_key : str, optional
            Override the active run.  Use the value returned by
            ``start_run()`` or from ``list_runs()``.
        """
        if not os.path.exists(self.path):
            raise FileNotFoundError(f"Catalog not found: {self.path}")
        key = self._resolve_param_key(run_key)
    ...
    def get_spectrum(self, objid: str, run_key: str | None = None) -> dict:
        """
        Retrieve all stored arrays for one object.

        Returns
        -------
        dict
            Keys are array names (``wave``, ``flux``, ``err``, ``conti``,
            ``line``, plus any extras).  Scalar attributes (ra, dec,
            redshift) are under the ``"attrs"`` key.
        """
        if run_key is not None:
            spectra_prefix = run_key.lstrip("/") + "/spectra"
        else:
            self._require_run()
            spectra_prefix = self._spectra_prefix

        grp_path = f"{spectra_prefix}/{objid}"
        with h5py.File(self.path, "r") as f:
            if grp_path not in f:
                raise KeyError(
                    f"No spectrum stored for '{objid}' under '{spectra_prefix}'"
                )
            grp = f[grp_path]
            result = {k: grp[k][:] for k in grp.keys()}
            result["attrs"] = dict(grp.attrs)
    ...
    def list_spectra(self, run_key: str | None = None) -> list[str]:
        ...
        with h5py.File(self.path, "r") as f:
            if spectra_prefix not in f:
                return []
            return list(f[spectra_prefix].keys())
    ...
    def list_runs(self) -> list[str]:
        """
        Return all run keys stored in the file, sorted chronologically.

        Each entry is a full path like ``"/20250601/20250601_143000_my_run"``.
        """
        if not os.path.exists(self.path):
            return []
        runs = []
...
pysofit_catalog.py
-------------------
A wrapper around pandas HDFStore for accumulating PyQSOFit results
across a batch run into a single HDF5 catalog.

Usage
-----
    from pyqsofit_catalog import QSOCatalog

    cat = QSOCatalog("my_run.h5")

    # After each fit:
    cat.append(qso.result, objid="J1234+5678")

    # Optionally store the spectrum too:
    cat.append(qso.result, objid="J1234+5678",
               wave=qso.wave, flux=qso.flux, err=qso.err,
               model=qso.Manorm)
    ...
    # Export to FITS for sharing:
    cat.to_fits("final_catalog.fits")

    # Check progress mid-run:
    print(cat)

    How to deal with sometimes having errors/different broad lines
"""
...
class QSOCatalog:
    ...
    def append(
        ...
        ----------
        result : dict
            The scalar output dict from PyQSOFit (ra, dec, redshift,
            PL_norm, MgII_br_1_scale, …).  Keys become column names.
        objid : str, optional
            A unique identifier for this object (e.g. "J1234+5678").
            If provided, it is stored as a column *and* used to name
            the spectrum group.  If omitted, no spectrum can be stored.
        wave, flux, err : np.ndarray, optional
            Observed wavelength / flux / error arrays.  Stored under
            ``spectra/<objid>/`` only when ``objid`` is given.
        conti : np.ndarray, optional
            Best-fit continuum model array (same length as wave).
        line: np.ndarray, optional
            Best-fit line model array (same length as wave).
        extra_arrays : dict of {str: np.ndarray}, optional
            Any additional 1-D arrays to store alongside the spectrum,
            e.g. ``{"fe_uv": qso.f_fe_uv, "conti": qso.f_conti}``.
        """
        result = _sanitize_result(result)
    ...
    def _store_spectrum(self, objid, *, wave, flux, err, model,
                        extra_arrays, attrs):
        with h5py.File(self.path, "a") as f:
            grp_path = f"{self.spectra_group}/{objid}"
            if grp_path in f:
                warnings.warn(
                    f"Spectrum for '{objid}' already exists — overwriting.",
                    stacklevel=3,
                )
                del f[grp_path]

            grp = f.require_group(grp_path)
            arrays = {"wave": wave, "flux": flux, "err": err, "conti": conti, }
            arrays.update(extra_arrays)

            for name, arr in arrays.items():
                if arr is not None:
                    grp.create_dataset(name, data=np.asarray(arr, dtype="f8"),
                                       compression="gzip", compression_opts=4)
    ...
    def _rebuild_with_new_row(self, df_new: pd.DataFrame) -> None:
        """
        Fallback: load the existing table, concatenate, and overwrite.
        Called only when a string column needs more space than was
        ...
        try:
            df_old = pd.read_hdf(self.path, key=self.param_key)
            df_combined = pd.concat([df_old, df_new], ignore_index=True)
        except KeyError:
            df_combined = df_new

        # Write combined back — use a temp key then rename
        df_combined.to_hdf(
            self.path,
            key=self.param_key,
            mode="a",
    ...
    def load(self, columns: list[str] | None = None) -> pd.DataFrame:
        """
        Load the full scalar parameter table.
        ...
        if not os.path.exists(self.path):
            raise FileNotFoundError(f"Catalog not found: {self.path}")
        return pd.read_hdf(self.path, key=self.param_key,
                           columns=columns)
    ...
    def query(self, condition: str,
        ...
        if not os.path.exists(self.path):
            raise FileNotFoundError(f"Catalog not found: {self.path}")
        return pd.read_hdf(self.path, key=self.param_key,
                           where=condition, columns=columns)

    def get_spectrum(self, objid: str) -> dict:
        """
        Retrieve all stored arrays for one object.
        ...
        -------
        dict
            Keys are array names (``wave``, ``flux``, ``err``, ``model``,
            plus any extras).  Attributes (ra, dec, redshift) are under
            the ``"attrs"`` key.
        """
        grp_path = f"{self.spectra_group}/{objid}"
        with h5py.File(self.path, "r") as f:
            if grp_path not in f:
                raise KeyError(f"No spectrum stored for '{objid}'")
            grp = f[grp_path]
            result = {k: grp[k][:] for k in grp.keys()}
            result["attrs"] = dict(grp.attrs)
        return result
    ...
    def list_spectra(self) -> list[str]:
        """Return a list of all object IDs that have stored spectra."""
        if not os.path.exists(self.path):
            return []
        with h5py.File(self.path, "r") as f:
            if self.spectra_group not in f:
                return []
            return list(f[self.spectra_group].keys())

    # ------------------------------------------------------------------
    # Export
    # ------------------------------------------------------------------

    def to_fits(self, output_path: str, overwrite: bool = True) -> None:
        """
        Export the scalar parameter table to a standard FITS BINTABLE.
        ...
        """
        df = self.load()
        # astropy can't handle 'object' dtype columns — convert to str
        for col in df.select_dtypes(include="object").columns:
            df[col] = df[col].astype(str)
        Table.from_pandas(df).write(output_path, overwrite=overwrite)
        print(f"Exported {len(df)} rows → {output_path}")
    ...
    def to_csv(self, output_path: str, **kwargs) -> None:
        """Export the scalar parameter table to CSV."""
        self.load().to_csv(output_path, index=False, **kwargs)
        print(f"Exported → {output_path}")
    ...
    def column_names(self) -> list[str]:
        """Return the list of stored parameter column names."""
        return list(self.load(columns=None).columns)

    def summary(self) -> pd.DataFrame:
        """Return basic statistics for all numeric columns."""
        return self.load().describe()
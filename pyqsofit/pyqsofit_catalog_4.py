    ...
    /YYYYMMDD/                              ← date group  (fit run date)
        YYYYMMDD_HHMMSS[_label]/            ← bulk_fit group (one per run)
            ...
            spectra/                        ← (full catalog only)
                <objid>/
                    <every non-empty numeric array attribute found on the
                     QSOFit object after Fit() completes>
                    attrs: ra, dec, z

    The set of arrays stored under spectra/<objid>/ is NOT a fixed list.
    It is discovered dynamically from whatever attributes happen to exist
    on the QSOFit instance (wave, flux, err, f_conti_model, f_line_model,
    f_pl_model, f_fe_mgii_model, f_fe_balmer_model, f_bc_model,
    f_poly_model, line_flux, host, qso, host_data, spec, ppxf_mask,
    ppxf_model, gauss_result_all, all_comp_range, ... whatever PyQSOFit
    happens to set). This means:
      - Attributes that don't exist for a given object (e.g. `.host` /
        `.qso` when decompose_host=False, or `.gauss_result_all` when
        MC=False and MCMC=False) are simply skipped — there is nothing
        to special-case.
      - New array attributes added in future PyQSOFit versions are picked
        up automatically, with no code changes needed here.
      - Non-numeric arrays (string/object arrays such as `comp_result`,
        `*_result_type`, `*_result_name` — these just duplicate column
        headers already present in the scalar `params` table) are
        skipped, since they can't be cast to float and add no value.
      - A small default exclude-list skips a couple of attributes that
        ARE numeric arrays but are constant template data shared by every
        object (fe_uv, fe_op), so they aren't pointlessly duplicated once
        per object. This is configurable via `start_run(exclude_attrs=...)`.

Usage
-----
    from pyqsofit_catalog import QSOCatalog

    # --- scalar-only catalog (fast) ---
    cat = QSOCatalog("params_only.h5")

    # --- full catalog (scalars + every array on the QSOFit object) ---
    ...
    cat_full.start_run(
        label="night1",
        extra_scalars={"instrument": "SDSS", "plate": 1234, "seeing": 1.2},
        # optional: widen/narrow the default array exclude list
        exclude_attrs={"fe_uv", "fe_op", "some_other_constant_array"},
    )
    ...
    cat_full.append(
        ...
        objid="J1234+5678",
        store_spectra=True,
        # optional: per-object scalars not produced by PyQSOFit itself
        extra_scalars={"logLbol": 46.2, "flag_broad": 1},
        # optional: per-object arrays not produced by PyQSOFit itself
        extra_arrays={"my_custom_mask": my_mask_array},
    )
...
def _host_pad_columns(npca_gal: int, npca_qso: int) -> list[str]:
    """
    Return the full list of host-related column names for a given PCA count.
    These are the columns that only appear when decompose_host=True succeeds,
    so we need to know them upfront to pad rows where decomposition failed.
    """
    cols = list(_HOST_RESULT_NAMES)
    cols += [f"gal_par_{i}" for i in range(npca_gal)]
    cols += [f"qso_par_{i}" for i in range(npca_qso)]
    return cols


def _qso_to_scalar_dict(qso, pad_columns: list[str] | None = None) -> dict:
    """
    ...
    - qso.host_result  / qso.host_result_name   — host decomposition scalars
      (read directly so they are present even when conti_result omits them)

    When decompose_host=False or decomposition failed, host_result is an
    empty array and host_result_name is empty.  pad_columns (pre-computed
    at start_run from npca_gal / npca_qso) lists every expected host column;
    any that are missing are filled with NaN so every row has the same schema.
    ...
    # Continuum results (ra, dec, redshift, SN_ratio_conti, PL params, etc.)
    ...
    # Emission line results (FWHM, sigma, EW, Gaussian params, etc.)
    ...
    # Host decomposition scalars — read directly from host_result so they
    # are always present when decomposition succeeded, regardless of whether
    # conti_result also included them.
    if hasattr(qso, 'host_result') and len(qso.host_result) > 0:
        for name, val in zip(qso.host_result_name, qso.host_result):
            try:
                result[name] = float(val)
            except (ValueError, TypeError):
                result[name] = str(val)

    # Pad any host columns that are absent (decomposition was skipped or
    # failed).  pad_columns covers the fixed 10 scalars + gal_par_N +
    # qso_par_N for the exact PCA counts declared at start_run().
    for name in (pad_columns or _HOST_RESULT_NAMES):
...
# ---------------------------------------------------------------------------
# Dynamic full-object array snapshot
# ---------------------------------------------------------------------------

# Numeric array attributes that are constant template data (identical for
# every object) rather than per-object results. Storing these once per
# object would be pure waste, so they're skipped by default. Override via
# start_run(exclude_attrs=...) if you want them included or want to add
# more names to skip.
_DEFAULT_ARRAY_EXCLUDE = frozenset({
    "fe_uv",   # UV FeII template (Boroson & Green 1992) — same for all objects
    "fe_op",   # optical FeII template (Vestergaard & Wilkes 2001) — same for all objects
})


def _qso_array_snapshot(qso, exclude: set | None = None) -> dict:
    """
    Walk every attribute on a QSOFit instance and return the subset that
    are non-empty numeric numpy arrays.

    This is what lets the catalog store "the whole object" without
    maintaining a hand-written list of array names: whatever attributes
    PyQSOFit happens to have set on `qso` after Fit() — wave, flux, err,
    f_conti_model, f_line_model, f_pl_model, f_fe_mgii_model,
    f_fe_balmer_model, f_bc_model, f_poly_model, line_flux, host, qso,
    host_data, spec, ppxf_mask, ppxf_model, gauss_result_all,
    all_comp_range, or anything added in a future PyQSOFit version — is
    captured automatically.

    Handling of edge cases
    -----------------------
    - Attribute doesn't exist at all (e.g. `.host` / `.qso` when
      decompose_host=False; `.gauss_result_all` when MC=MCMC=False):
      it's simply absent from vars(qso), so it's absent from the result.
      No AttributeError, no special-casing needed.
    - Attribute exists but is an empty array (e.g. `.host_result` when
      decomposition was skipped/failed): skipped via the size==0 check.
    - Attribute is a non-numeric array (e.g. `.comp_result`,
      `.conti_result_type`, `.gauss_result_name` — arrays of strings that
      just duplicate column headers already in the scalar `params`
      table): the float cast fails and it's skipped.
    - Attribute isn't an ndarray at all (lmfit fit results, the
      `linelist` Table, matplotlib Figure, plain scalars/strings/bools):
      skipped by the isinstance check. Scalars are already captured by
      `_qso_to_scalar_dict` via conti_result/line_result/host_result.

    Parameters
    ----------
    qso : QSOFit
        A QSOFit object after qso.Fit(...) has been called.
    exclude : set of str, optional
        Extra attribute names to skip, merged with _DEFAULT_ARRAY_EXCLUDE.

    Returns
    -------
    dict
        {attribute_name: float64 ndarray}
    """
    skip = _DEFAULT_ARRAY_EXCLUDE | (exclude or set())
    arrays = {}
    for name, val in vars(qso).items():
        if name in skip:
            continue
        if not isinstance(val, np.ndarray) or val.size == 0:
            continue
        try:
            arrays[name] = np.asarray(val, dtype="f8")
        except (TypeError, ValueError):
            # Non-numeric array (names/labels/etc.) — nothing useful to store.
            continue
    return arrays
...
class QSOCatalog:
    ...
    The internal hierarchy is::
        ...
        /YYYYMMDD/
            YYYYMMDD_HHMMSS[_label]/
                ...
                spectra/        ← per-object arrays (full catalog only)
                    <objid>/
                        <every non-empty numeric array attribute found
                         on the QSOFit object — see _qso_array_snapshot>
    ...
    def __init__(self, path: str):
        ...
        # Per-object array keys declared on the first append() call that
        # supplied extra_arrays. Same consistency rule as _per_object_keys.
        self._per_object_array_keys: set | None = None

        # Pre-computed list of host column names (fixed + gal_par_N + qso_par_N)
        # built at start_run() so non-decomposed objects can be padded correctly.
        self._host_pad_cols: list[str] = list(_HOST_RESULT_NAMES)

        # Extra attribute names to exclude from the dynamic array snapshot,
        # on top of _DEFAULT_ARRAY_EXCLUDE. Set by start_run().
        self._exclude_attrs: set = set()
    ...
    def start_run(
        ...
        npca_gal : int, optional
            Number of galaxy PCA components passed to QSOFit.Fit().
            Must match the npca_gal used in the fitting loop so that
            gal_par_0 … gal_par_{npca_gal-1} columns are always present,
            even for objects where decomposition was skipped or failed.
            Default: 5 (PyQSOFit default).
        npca_qso : int, optional
            Number of QSO PCA components passed to QSOFit.Fit().
            Same rationale as npca_gal.  Default: 10 (PyQSOFit default).
        exclude_attrs : set of str, optional
            Extra QSOFit attribute names to skip when storing the full
            per-object array snapshot (store_spectra=True). Merged with
            the built-in default exclude set (currently just the constant
            FeII templates `fe_uv`/`fe_op`). Use this if you want to skip
            other large/uninteresting arrays for this run, e.g.
            ``{"ppxf_model"}``. Has no effect on the scalar-only table.
        ...
        self._per_object_array_keys = None    # reset; first append will declare keys
        self._host_pad_cols   = _host_pad_columns(npca_gal, npca_qso)
        self._exclude_attrs   = set(exclude_attrs or set())
    ...
    def append(
        ...
        store_spectra : bool, optional
            If True, store every non-empty numeric array found on the
            QSOFit object under spectra/<objid>/ (wave, flux, err,
            f_conti_model, host, qso, ppxf_mask, gauss_result_all,
            whatever happens to be present — see module docstring).
        ...
        extra_arrays : dict, optional
            Per-object arrays to store alongside whatever is auto-detected
            on the QSOFit object, e.g. ``{"my_mask": my_mask_array}``.
            Only used when store_spectra=True. If a key collides with an
            attribute name already found on the QSOFit object, the
            extra_arrays value wins and a warning is raised. Like
            extra_scalars, the key set declared on the first append()
            that supplies extra_arrays is enforced on every later append
            in the run.
        """
        self._require_run()

        # Build scalar dict from the QSOFit result arrays
        result = _qso_to_scalar_dict(qso, pad_columns=self._host_pad_cols)
        result['objid'] = objid
        ...
        # ---- enforce extra_arrays schema consistency ------------------
        # (checked here, before any writes, so a bad call fails atomically)
        if store_spectra:
            if extra_arrays:
                if self._per_object_array_keys is None:
                    self._per_object_array_keys = set(extra_arrays.keys())
                else:
                    missing = self._per_object_array_keys - set(extra_arrays.keys())
                    extra   = set(extra_arrays.keys()) - self._per_object_array_keys
                    if missing or extra:
                        raise ValueError(
                            f"extra_arrays keys do not match the schema declared "
                            f"on the first append of this run.\n"
                            f"  Expected : {sorted(self._per_object_array_keys)}\n"
                            f"  Got      : {sorted(extra_arrays.keys())}\n"
                            f"  Missing  : {sorted(missing)}\n"
                            f"  Unexpected: {sorted(extra)}"
                        )
            elif self._per_object_array_keys is not None:
                raise ValueError(
                    f"This append() call is missing per-object extra_arrays. "
                    f"Expected keys: {sorted(self._per_object_array_keys)}"
                )
        ...
        # ---- full per-object array snapshot ---------------------------
        if store_spectra:
            self._store_spectrum(qso, objid, extra_arrays=extra_arrays)

    def _store_spectrum(self, qso, objid: str, extra_arrays: dict | None = None) -> None:
        """
        Write a full per-object array snapshot into spectra/<objid>/
        within the active run group.

        The set of arrays stored is discovered dynamically from whatever
        attributes are present on `qso` (see `_qso_array_snapshot` for the
        exact rules — missing attributes, empty arrays, and non-numeric
        arrays are all skipped automatically; the default exclude list and
        any extra `exclude_attrs` passed to `start_run()` are also applied).
        `extra_arrays`, if given, are merged in on top (and take priority
        over same-named attributes auto-detected from `qso`).
        """
        self._require_run()
        grp_path = f"{self._spectra_prefix}/{objid}"

        arrays = _qso_array_snapshot(qso, exclude=self._exclude_attrs)

        if extra_arrays:
            shadowed = [k for k in extra_arrays if k in arrays]
            if shadowed:
                warnings.warn(
                    f"extra_arrays key(s) {shadowed} shadow QSOFit attributes "
                    f"of the same name for object '{objid}' — the extra_arrays "
                    f"version will be stored.",
                    stacklevel=3,
                )
    ...
    def get_spectrum(
        ...
        """
        Retrieve every stored array for one object.

        Returns
        -------
        dict
            Keys are whatever array names happened to be stored for this
            object (``wave``, ``flux``, ``err``, ``f_conti_model``, ``host``,
            ``qso``, etc. — see module docstring for how this set is
            determined). Scalar metadata (ra, dec, z) are under the
            ``"attrs"`` key.
        """
        if run_key is not None:
